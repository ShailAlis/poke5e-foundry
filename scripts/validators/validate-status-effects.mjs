...
assert.equal(burnedSource.icon, "modules/poke5e-foundry/assets/icons/effects/statuses/burned.png");
assert.equal(burnedSource.img, burnedSource.icon);
assert.deepEqual(burnedSource.statuses, ["poke5e-foundry-burned"]);
const { registerPokemonStatusEffects } = await import("../combat/status-effects.mjs");
...
const { registerPokemonStatusSocket } = await import("../combat/status-effects.mjs");
