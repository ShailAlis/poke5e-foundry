...
export const CONTEXTUAL_MODIFIER_COVERAGE = Object.freeze({
  ...
  payback: "Dobla los dados si el usuario recibió daño desde que acabó su último turno: gestionado por combat-history.mjs, aproximado a 'cualquier daño recibido' porque este proyecto no rastrea qué actor concreto infligió cada golpe.",
  avalanche: "Mismo motivo que payback.",
  "bolt-beak": "Dobla el modificador MOVE si el último movimiento del objetivo (instance.lastMoveId) no fue un movimiento de ataque: consultado directamente en pokemon-sheet.mjs, no una regla de este catálogo.",
  "fishious-rend": "Mismo motivo que bolt-beak.",
  "stomping-tantrum": "Dobla los dados si el propio último ataque del usuario falló: gestionado por combat-history.mjs/instance.lastAttackMissed.",
  "temper-flare": "Añade un dado extra (no lo dobla) si el propio último ataque del usuario falló: mismo rastreo que stomping-tantrum, con addExtraDie() en multi-hit.mjs.",
  assurance: "Dobla los dados si el objetivo ya recibió daño esta misma ronda: gestionado por combat-history.mjs (damagedThisRound, reiniciado cada ronda).",
  "pay-day": "Otorga 10×nivel ₽ al entrenador la primera vez que impacta a cada objetivo en el combate: resuelto directamente en pokemon-sheet.mjs (economy.mjs), con una bandera de un solo uso por objetivo en su propio actor.",
  "make-it-rain": "Reparte ₽ al entrenador según cuántos objetivos fallan o superan la salvación: resuelto directamente en pokemon-sheet.mjs reutilizando rollFailedSaves().",
  "happy-hour": "Duplica el dinero de premio al final del combate, pero este proyecto no calcula ni reparte premios de combate automáticamente (se hace a mano); no hay nada que doblar. Se resuelve manualmente.",
  thief: "Roba el objeto equipado del objetivo tras una salvación de DES fallida solo si el usuario no lleva ninguno: resuelto directamente en pokemon-sheet.mjs reutilizando requestHeldItemSwap() de item-swap.mjs (el objetivo se queda sin nada, igual que calcula swapHeldItems() cuando el origen no lleva objeto).",
  incinerate: "Destruye la baya equipada del objetivo si la lleva: gestionado por item-swap.mjs (requestHeldItemDestroy, isBerryHeldItem), no una regla de este catálogo.",
  "knock-off": "Retira el objeto equipado del objetivo hasta el final del combate y lo devuelve al terminar: gestionado por item-swap.mjs (requestHeldItemDestroy con restoreAfterCombat, restaurado en el hook deleteCombat).",
  "bug-bite": "Como Thief pero además el usuario consume la baya robada y obtiene su efecto de curación/estado en el mismo golpe: exigiría invocar la reacción de baya de held-items.mjs (pensada para que el propio dueño la coma) sobre un objeto que en ese instante ya no es del objetivo ni del usuario. Se resuelve manualmente.",
  pluck: "Mismo motivo que bug-bite.",
  covet: "Roba el objeto del objetivo solo si el usuario no lleva ninguno, igual que Thief, pero el texto añade condiciones adicionales sobre objetos no robables (Objetos Misteriosos, objetos de trama) que este catálogo de objetos equipados no distingue. Se resuelve manualmente.",
  fling: "El daño y el efecto secundario dependen de qué objeto concreto lleve el usuario equipado (tabla larga por objeto) y lo consume tras usarlo: no hay una tabla de 'efecto de Fling por objeto' en held-items.mjs. Se resuelve manualmente.",
  "natural-gift": "El tipo y la potencia del movimiento dependen de qué baya lleve el usuario equipada (tabla por baya) y la consume: mismo motivo que fling, sin tabla equivalente en held-items.mjs. Se resuelve manualmente.",
  "techno-blast": "El tipo de daño depende del Drive que lleve equipado el usuario: no hay catálogo de Drives en held-items.mjs (son objetos exclusivos de Genesect). Se resuelve manualmente.",
  judgment: "El tipo de daño depende de la Placa que lleve equipada el usuario: no hay catálogo de Placas en held-items.mjs (son objetos exclusivos de Arceus). Se resuelve manualmente.",
  "hidden-power": "El tipo de daño sale de la tabla de 1d20 propia del movimiento: resuelto directamente en pokemon-sheet.mjs (random-tables.mjs, hiddenPowerType), no una regla de este catálogo. El resultado 20 ('a elección') cae de vuelta al diálogo normal de tipo.",
  magnitude: "Los dados de daño salen de la tabla de d100 propia del movimiento, independiente del nivel: resuelto directamente en pokemon-sheet.mjs (random-tables.mjs, magnitudeDice). El daño doble a criaturas enterradas/en Cavar y la inmunidad de las 'Alzadas' no se comprueban, igual que el resto de excepciones de invulnerabilidad decorativa.",
  acupressure: "Tira en su propia tabla de 1d6 y aplica un modificador dinámico distinto según el resultado: resuelto con #rollAcupressure() en pokemon-sheet.mjs y applyDynamicModifier() en move-modifiers.mjs, no una entrada estática de este catálogo.",
  "secret-power": "El estado que causa en natural 15+ sale de una tabla de 1d6 propia: resuelto directamente en pokemon-sheet.mjs construyendo un move.statusEffects sintético para reutilizar applyMoveStatuses() tal cual.",
  "tri-attack": "Mismo motivo que secret-power, pero con salvación fallida por margen de 5. El texto original tira 1d6 por cada objetivo que falla; este proyecto simplifica a una sola tirada compartida para toda la activación.",
  metronome: "Ejecuta un movimiento completamente aleatorio de los 830 del catálogo con las estadísticas propias: exigiría reproducir el pipeline entero de #rollMove() para un movimiento elegido al azar en tiempo de ejecución, con el riesgo de que ese movimiento a su vez dependa de objetivos, terreno o clima incompatibles con la situación actual. Se resuelve manualmente.",
  "stealth-rock": "Trampa de entrada: se dispara como reacción cada vez que el entrenador RIVAL despliega un Pokémon nuevo, indefinidamente hasta que la trampa se retire. deployment.mjs no modela bandos/equipos (solo qué Pokémon pertenece a qué entrenador), así que no hay forma de distinguir 'el entrenador contrario' de quien active la trampa sin construir ese concepto antes. Se resuelve manualmente.",
  spikes: "Mismo motivo que stealth-rock.",
  "sticky-web": "Mismo motivo que stealth-rock.",
  "toxic-spikes": "Mismo motivo que stealth-rock.",
  soak: "Cambia el tipo del objetivo a Agua durante la duración: mismo riesgo que conversion/conversion-2 de no tener una forma segura de revertir un tipo de criatura sustituido temporalmente. Se resuelve manualmente.",
  "trick-or-treat": "Mismo motivo que soak, cambiando a tipo Fantasma.",
  "forests-curse": "Mismo motivo que soak, cambiando a tipo Planta.",
  "magic-powder": "Mismo motivo que soak, cambiando a tipo Psíquico.",
  "reflect-type": "El usuario copia sobre sí mismo el tipo de una criatura objetivo: mismo riesgo que soak, pero sobre el propio usuario en vez del objetivo.",
  synchronoise: "Solo daña a las criaturas que comparten al menos un tipo con el usuario: exigiría filtrar los objetivos seleccionados por tipo antes de resolver el ataque, algo que este catálogo de modificadores no hace (decide efectos posteriores al ataque, no a quién alcanza). Se resuelve manualmente."
});
